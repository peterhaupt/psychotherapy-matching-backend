...
## Summary
Streamlined infrastructure service handling all emails and phone calls for both therapists and patients. Focuses on communication delivery with markdown support, legal compliance features, dual recipient architecture, and synchronous updates to patient records.
...
- **Direct API Integration**: Synchronous updates to Patient Service
...
### With Patient Service (Direct API Integration)
Comprehensive patient communication support with synchronous updates:
- **Helper functions**: Pre-built utilities in `patient_service/utils/communication.py`
- **Automatic tracking**: Direct API calls to update `letzter_kontakt` field
...
- **Email sent**: Calls `PATCH /api/patients/{id}/last-contact` when email sent
- **Email response**: Updates patient when response received
- **Phone call completed**: Updates patient after successful call
- **Graceful degradation**: Logs errors but continues if Patient Service unavailable
...
- **Contact date tracking**: Updates handled by Therapist Service internally
...
- **Service URLs**: Patient and Therapist service endpoints for API calls

## API Client Integration

### Retry Logic
The service uses `shared.api.retry_client.RetryAPIClient` for resilient communication:
- Automatic retry on network failures to Patient Service
- Exponential backoff strategy
- Configurable timeout settings
- Non-blocking: Failures logged but don't prevent communication operations

### Patient Update Flow
When communication events occur:
1. **Email Sent**: Update patient last contact via API
2. **Email Response Received**: Update patient last contact
3. **Phone Call Completed**: Update patient last contact
4. **Failure Handling**: Log errors but continue operation
...
- **Communication only**: Focused on reliable message delivery
- **Content agnostic**: Accepts content from requesting services
- **Transport layer**: Handles delivery and status tracking
...
This streamlined architecture provides reliable communication infrastructure while maintaining flexibility for the specific needs of the therapy matching platform, now with direct synchronous integration to maintain data consistency across services.